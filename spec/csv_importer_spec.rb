...
module CSVHelper
  describe CSVImporter do
    ...
    it "complains about required rows missing" do
      ...
      expect { CSVImporter.import "spec/fixtures/sample_csv.csv", headers_required }.to raise_error RuntimeError, "Field(s) col_missing_1, col_missing_2 missing in the CSV file spec/fixtures/sample_csv.csv"
    end

    it "doesn't complain if the csv file contains additional rows not required" do
      headers_required = ["col_a1", "col_a4"]
      CSVImporter.import("spec/fixtures/sample_csv.csv", headers_required)
    end
